...
document.getElementById('level').innerHTML = "Level: " + character[0].level;
document.getElementById('experience').innerHTML = "Experience: " + character[0].experience;
...
let Prologue = function startGame() {
    ...
    player = new component(40, 50, "white", 10, 120);
    ...
    this.render = function () {
        ...
        if (27 in keyMap) // Esc - Pause Game  
        {
            Game.pauseGame();
        }
...
function aiCollision(r1, r2) {
    ...
    let crossWidth = width * dy;
    let crossHeight = height * dx;
    let aiCollision = 'none';
    //
    if (Math.abs(dx) <= width && Math.abs(dy) <= height) {
        if (crossWidth > crossHeight) {
            aiCollision = (crossWidth > (-crossHeight)) ? 'bottom' : 'left';
        } else {
            aiCollision = (crossWidth > -(crossHeight)) ? 'right' : 'top';
        }
    }
    side = aiCollision;
    return (aiCollision);
...
function checkLevel() {
    if (character[0].experience >= xpNeeded) {
        xpNeeded *= 3;
        level++;
        character[0].healh = 100;
        character[0].level++;
        document.getElementById('level').innerHTML = "Level: " + character[0].level;
        document.getElementById('experience').innerHTML = "Experience: " + character[0].experience;
        console.log("LEVEL UP! " + character[0].level);
    }
}
